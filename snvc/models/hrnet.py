"""
High-resolution feature extraction module for left/right regions of interest.

Adapted from https://github.com/HRNet/deep-high-resolution-net.pytorch
"""
...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self, cfg, **kwargs):
        ...
        if "head_type" in kwargs and kwargs["head_type"] == "heatmap_regression":
            pass
        else:
        ...
        self.stage1_cfg = cfg.extra.stage1
        ...
        elif self.head_type == "heatmap_regression":
            # Add a pixel shuffle upsampling layer to control the heatmap size
            self.upsamp_fact = 2 
            self.final_layer_hm = nn.Sequential(nn.Conv2d(pre_stage_channels[0], 
                                                       9*self.upsamp_fact**2, 
                                                       kernel_size=1,
                                                       stride=1,
                                                       padding=0
                                                       ),                
    ...
    def forward(self, x):
        if self.head_type != "heatmap_regression":
            x = self.conv1(x)
            x = self.bn1(x)
            x = self.relu(x)
            x = self.conv2(x)
        ...
        if self.head_type == "classification":
            ...
            y = self.classifier(y)
        elif self.head_type == "default":
            y = y_list[0]
        elif self.head_type == "heatmap_regression":
            y = y_list[0]
            y = self.final_layer_hm(y)
        return y

    def init_weights(self, pretrained='',):
        logger.info('=> init weights for 2D feature extraction from normal distribution')
...
def get_model(cfg, is_train, **kwargs):
    ...
    if is_train and cfg.init_weights:
        model.init_weights(cfg.pre_trained_path)

    if hasattr(cfg, "add_xy") and cfg.add_xy:
