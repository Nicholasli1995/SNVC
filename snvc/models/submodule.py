...
def get_hg_down_sample(channel_in, channel_out, gn, downsample=True):
    stride = 2 if downsample else 1
    module = nn.Sequential(convbn_3d(channel_in, 
                                     channel_out, 
                                     kernel_size=3,
                                     stride=stride,
                                     pad=1,
...
def get_hg_down_sample_2d(channel_in, channel_out, gn, downsample=True):
    stride = 2 if downsample else 1
    module = nn.Sequential(convbn(channel_in, 
                                  channel_out, 
                                  kernel_size=3,
                                  stride=stride,
                                  pad=1,
...
def get_hg_up_sample(channel_in, channel_out, gn):
    module = nn.Sequential(nn.ConvTranspose3d(channel_in, 
                                              channel_out,
                                              kernel_size=3, 
                                              padding=1, 
                                              output_padding=1, 
                           ...
                           nn.BatchNorm3d(channel_out) if not gn else nn.GroupNorm(32, channel_out)
                           )    
    return module

def get_hg_up_sample_2d(channel_in, channel_out, gn):
    module = nn.Sequential(nn.ConvTranspose2d(channel_in, 
                                              channel_out,
                                              kernel_size=3, 
                                              padding=1, 
                                              output_padding=1, 
                           ...
                           nn.BatchNorm2d(channel_out) if not gn else nn.GroupNorm(32, channel_out)
                           )    
    return module

class hourglass_downsample_16(nn.Module):
    def __init__(self, inplanes, gn=False):
        super(hourglass_downsample_16, self).__init__()

        self.conv1 = get_hg_down_sample(inplanes, inplanes*2, gn)
        self.conv2 = get_hg_down_sample(inplanes*2, inplanes*2, gn, False)
        
        self.conv3 = get_hg_down_sample(inplanes*2, inplanes*2, gn)
        self.conv4 = get_hg_down_sample(inplanes*2, inplanes*2, gn, False)
        
        self.conv5 = get_hg_down_sample(inplanes*2, inplanes*2, gn)
        self.conv6 = get_hg_down_sample(inplanes*2, inplanes*2, gn, False)        

        self.conv7 = get_hg_down_sample(inplanes*2, inplanes*2, gn)
        self.conv8 = get_hg_down_sample(inplanes*2, inplanes*2, gn, False)  
        
        self.conv9 = get_hg_up_sample(inplanes*2, inplanes*2, gn)
        self.conv10 = get_hg_up_sample(inplanes*2, inplanes*2, gn)
        self.conv11 = get_hg_up_sample(inplanes*2, inplanes*2, gn)
        
        self.conv12 = get_hg_up_sample(inplanes*2, inplanes, gn)
        
    def forward(self, x):
        out_conv1 = self.conv1(x)  # in:1 out:1/2
        out_conv2 = self.conv2(out_conv1)  # in:1/2 out:1/2
        
        out_conv3 = self.conv3(out_conv2) # in:1/2 out:1/4
        out_conv4 = self.conv4(out_conv3) # in:1/4 out:1/4
        
        out_conv5 = self.conv5(out_conv4) # in:1/4 out:1/8
        out_conv6 = self.conv6(out_conv5) # in:1/8 out:1/8

        out_conv7 = self.conv7(out_conv6) # in:1/8 out:1/16
        out_conv8 = self.conv8(out_conv7) # in:1/16 out:1/16
        
        out_conv9 = self.conv9(out_conv8) # in:1/16 out:1/8
        inp_conv10 = out_conv9 + out_conv6
        
        out_conv10 = self.conv10(inp_conv10) # in:1/8 out:1/4
        inp_conv11 = out_conv10 + out_conv4
        
        out_conv11 = self.conv11(inp_conv11)  # in:1/4 out:1/2
        
        out = self.conv12(out_conv11 + out_conv2) # in:1/2 out:1

        return out

class hourglass2d_downsample_16(nn.Module):
    def __init__(self, inplanes, gn=False):
        super(hourglass2d_downsample_16, self).__init__()

        self.conv1 = get_hg_down_sample_2d(inplanes, inplanes*2, gn)
        self.conv2 = get_hg_down_sample_2d(inplanes*2, inplanes*2, gn, False)
        
        self.conv3 = get_hg_down_sample_2d(inplanes*2, inplanes*2, gn)
        self.conv4 = get_hg_down_sample_2d(inplanes*2, inplanes*2, gn, False)
        
        self.conv5 = get_hg_down_sample_2d(inplanes*2, inplanes*2, gn)
        self.conv6 = get_hg_down_sample_2d(inplanes*2, inplanes*2, gn, False)        

        self.conv7 = get_hg_down_sample_2d(inplanes*2, inplanes*2, gn)
        self.conv8 = get_hg_down_sample_2d(inplanes*2, inplanes*2, gn, False)  
        
        self.conv9 = get_hg_up_sample_2d(inplanes*2, inplanes*2, gn)
        self.conv10 = get_hg_up_sample_2d(inplanes*2, inplanes*2, gn)
        self.conv11 = get_hg_up_sample_2d(inplanes*2, inplanes*2, gn)
        
        self.conv12 = get_hg_up_sample_2d(inplanes*2, inplanes, gn)
