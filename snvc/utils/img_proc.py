...
def get_affine_transform(
    ...
    scale_tmp = scale * SIZE if not absolute else scale
    src_w = scale_tmp[0]
    dst_w = output_size[0]
    dst_h = output_size[1]
...
def get_affine_trans_center_crop(bbox, resolution):
    affine_mat = np.zeros((2, 3), dtype=np.float32)
    x1, y1, x2, y2 = bbox
    affine_mat[0,0] =  resolution[0] / (x2 - x1)
    affine_mat[0,2] = - affine_mat[0,0] * x1
    affine_mat[1,1] = resolution[1] / (y2 - y1)
    affine_mat[1,2] = - affine_mat[1,1] * y1        
    return affine_mat
    
def affine_transform(kpts_2d, trans, dtype=np.float32):
    # local coordinates within the patch
    homo = np.concatenate([kpts_2d, np.ones((len(kpts_2d), 1))], axis=1)
    return (trans @ homo.T).astype(dtype)
...
def cs2bbox(center, size, np_array=False):
    x1 = center[0] - size[0]
    y1 = center[1] - size[1]
    x2 = center[0] + size[0]
    y2 = center[1] + size[1]
...
def kpts2cs(keypoints, 
    ...
    if method == 'centroid':
        ...
        crop_size = ((right_bottom - left_top)*enlarge).squeeze()
    else:
...
def soft_arg_max(batch_heatmaps, 
    ...
    # normalize the heatmaps so that they sum to 1
    heatmaps_reshaped = torch.exp(F.relu(heatmaps_reshaped - 0.01)) - 1
    denominator = heatmaps_reshaped.sum(dim=2, keepdim=True)
    normalized_heatmaps = heatmaps_reshaped / denominator
    # heatmaps_reshaped = F.softmax(heatmaps_reshaped, dim=2)
    batch_heatmaps = normalized_heatmaps.view(batch_size, num_joints, height, width)
    x = batch_heatmaps.sum(dim = 2)
    y = batch_heatmaps.sum(dim = 3)
    if end is None:
        end = (width, height)
    # x_indices = torch.arange(width).type(torch.cuda.FloatTensor)
    x_indices = torch.arange(start[0], end[0], step[0], dtype=datatype, device=device)
    ...
    y_indices = torch.arange(start[1], end[1], step[1], dtype=datatype, device=device)
