...
def compute_locations_bev(Z_MIN, 
                          Z_MAX, 
                          VOXEL_Z_SIZE, 
                          X_MIN, 
                          X_MAX, 
    ...
    shifts_z = torch.arange(Z_MIN, 
                            Z_MAX - np.sign(VOXEL_Z_SIZE) * 1e-10, 
                            step=VOXEL_Z_SIZE, 
                            dtype=torch.float32
                            ).to(device) + VOXEL_Z_SIZE / 2.
    shifts_x = torch.arange(X_MIN, 
                            X_MAX - np.sign(VOXEL_X_SIZE) * 1e-10, 
                            step=VOXEL_X_SIZE, 
                            dtype=torch.float32
                            ).to(device) + VOXEL_X_SIZE / 2. # Z_min <= z < Z_max
